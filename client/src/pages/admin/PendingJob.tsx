  ...
  return (
    ...
    <div className="gap-4 grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1  p-4 text-prettyGray">
        ...
        <div className="flex-1  ">
          ...
          <div className="text-sm flex-wrap text-gray-300 bg-gray-700/10 p-3 rounded-md h-40">
            {job?.skills?.map((skill) => (
              <span key={skill}
                    className="inline-block bg-gray-600/30 text-gray-200 px-2 py-1 m-1 rounded-md">{separateCamelCase(skill)}</span>
            ))
            }
          </div>
        </div>
        <div className="flex-1  ">
          ...
          <div className="text-sm flex-wrap text-gray-300 bg-gray-700/10 p-3 rounded-md h-40">
            {job?.benefits?.map((benefit) => (
              <span key={benefit}
                    className="inline-block bg-gray-600/30 text-gray-200 px-2 py-1 m-1 rounded-md">{separateCamelCase(benefit)}</span>
            ))
            }
          </div>
        </div>
      ...
      <div className="px-8 py-2 flex gap-4">
        <button onClick={()=>reviewJob(1)} className="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-md transition-colors duration-200"
                type="button">
          Approve </button>
        <button onClick={()=>reviewJob(2)} className="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-md transition-colors duration-200"
                type="button">
          Reject
        </button>
